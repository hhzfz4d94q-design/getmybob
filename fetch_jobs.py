...
<style>
  ...
  .contact-row .cname {{ font-weight: 700; color: #1f3a5f; font-size: 14px; }}
  ...
  .contact-summary {{ background:#f3f7fc; border:1px solid #d6e1f1; border-radius:8px; padding:10px 14px; font-size:12px; color:#1f3a5f; margin-bottom:14px; display:flex; justify-content:space-between; align-items:center; gap:10px; flex-wrap:wrap; }}
  ...
  document.querySelectorAll('.card').forEach(card => {{
    ...
    if (st) {{
      if (!detailsBtn) {{
        ...
        detailsBtn.style.cssText = 'background:#f3f4f6;color:#1f3a5f;border:1px solid #ccd0d6;';
...
function renderFunnel(counts) {{
  ...
  function stage(label, count, prevCount, color) {{
    ...
    return '<div class="funnel-stage" style="flex:1; background:' + color + '; padding:10px 14px; border-radius:8px; color:#1f3a5f; min-width:0;">' +
...
function _renderInterviewPrep(qs) {{
  ...
  return qs.map((q, i) => {{
    ...
    return '<div class="prep-section">' +
      '<div class="prep-label">' + (i+1) + '. ' + _esc(q.q || '') + (type ? ' <span style="background:#eef; color:#1f3a5f; padding:1px 7px; border-radius:10px; font-size:10px; margin-left:6px;">' + type + '</span>' : '') + '</div>' +
...
function _renderProfileHTML(p) {{
  function section(title, items, color, fieldKey) {{
    ...
    const chips = items.map(s => {{
      ...
      return '<span style="display:inline-flex;align-items:center;background:' + color + ';color:#1f3a5f;padding:3px 4px 3px 9px;border-radius:12px;font-size:11.5px;margin:2px 4px 2px 0;font-weight:500;">' +
             safe +
    ...
    const addBtn = fieldKey
      ? '<button onclick="addChip(\\''+ fieldKey +'\\')" style="display:inline-block;background:#fafbfc;border:1px dashed #c0c8d4;color:#1f3a5f;padding:3px 11px;border-radius:12px;font-size:11.5px;cursor:pointer;font-weight:500;margin:2px 4px 2px 0;">+ Add</button>'
      : '';
...
function renderContactsList() {{
  ...
  companies.forEach(normCo => {{
    ...
    html += '<div style="font-weight:700;color:#1f3a5f;font-size:13px;margin-bottom:6px;">' + _escHtml(display) + ' <span style="color:#888;font-weight:normal;font-size:11px;">(' + list.length + ')</span></div>';
    list.forEach(c => {{
...
function openCompanyContactsModal(company, jobTitle, jobUrl, contacts) {{
  ...
  contacts.forEach((c, i) => {{
    ...
    if (c.email) {{
      ...
      html += '<a class="btn" href="mailto:' + _escHtml(c.email) + '?subject=' + subj + '&body=' + body + '" style="text-decoration:none;background:#f3f4f6;color:#1f3a5f;border:1px solid #ccd0d6;">Email instead</a>';
    }}
    html += '</div>';
    html += '</div>';
  }});
